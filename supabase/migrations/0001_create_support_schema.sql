...
create type support.platform as enum ('telegram', 'slack');
...
create type support.action_type as enum (
...
create type support.escalation_method as enum ('telegram_voice', 'phone_call');

-- ============================================================================
-- Helper function to manage updated_at columns
-- ============================================================================
create or replace function support.fn_set_updated_at()
...
-- ENGINEERS
-- ============================================================================
create table support.engineers (
...
create trigger trg_engineers_updated
  before update on support.engineers
  for each row execute procedure support.fn_set_updated_at();
...
create table support.tickets (
  id                 uuid primary key default gen_random_uuid(),
  platform           support.platform not null,
  ...
  current_engineer_id uuid references support.engineers(id) on delete set null,
...
create index idx_tickets_status_created on support.tickets(status, received_at desc);
...
create table support.ticket_actions (
  ...
  action_type       support.action_type not null,
  actor_engineer_id uuid references support.engineers(id) on delete set null,
  actor_external_id text,             -- For user messages: Slack/TG user id
  escalation_method support.escalation_method,
...
create index idx_ticket_actions_ticket on support.ticket_actions(ticket_id, created_at); 